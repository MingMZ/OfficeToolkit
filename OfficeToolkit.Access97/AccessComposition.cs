...
namespace OfficeToolkit.Access97
{
    [AddIn("Access '97 Composition", Version = "0.5.0.0")]
    public class AccessComposition : IAccessComposition
    {
        ...
        public void SaveObjects(string baseDirectory)
        {
            ...
            {
                _application.Application.SaveAsText((int)AcObjectType.acQuery, td.Name, Path.Combine(baseDirectory, di.Name, td.Name + ".txt"));
            }
        }
        ...
        private void SaveObjects(string baseDirectory, string subDirectoryName, Container acContainer, AcObjectType acType)
        {
            ...
            {
                _application.Application.SaveAsText((int)acType, document.Name, Path.Combine(baseDirectory, di.Name, document.Name + ".txt"));
            }
        }
        ...
        public void LoadObjects(string baseDirectory)
        {
            ...
            {
                _application.Application.LoadFromText((int)AcObjectType.acQuery, Path.GetFileNameWithoutExtension(f.Name), f.FullName);
            }
        }
        ...
        private void LoadObjects(string baseDirectory, string subDirectoryName, Container acContainer, AcObjectType acType)
        {
            ...
            foreach (FileInfo f in files)
            {
                _application.LoadFromText((int)acType, Path.GetFileNameWithoutExtension(f.Name), f.FullName);
            }
        }
